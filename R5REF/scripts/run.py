...
#     print 'addr = ', addr 
    ...
    # run Gse GUI
    GUI_args = [python_bin,"%s/Gse/bin/gse.py"%build_root,"--port","%d"%used_port,"--dictionary","%s/R5REF/py_dict"%build_root,"--connect","--addr",addr,"-L","%s/R5REF/logs"%build_root]
    ...
    bin_path = "%s/R5REF/%s/R5REF"%(build_root,os.environ["OUTPUT_DIR"])
    
    # run as root
    if not nobin:
        bin_args = [python_bin, "%s/Gse/bin/pexpect_runner.py"%build_root,"R5REF.log","R5REF Application","sudo",bin_path,"-p","%d"%used_port,"-a",addr]
