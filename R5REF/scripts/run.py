...
#     print 'addr = ', addr 
    ...
    # run Gse GUI
    GUI_args = [python_bin,"%s/Gse/bin/gse.py"%build_root,"--port","%d"%used_port,"--dictionary","%s/Gse/generated/R5REF"%build_root,"--connect","--addr",addr,"-L","%s/R5REF/logs"%build_root]
    ...
    BIN = "%s/R5REF/%s/R5REF"%(build_root,os.environ["OUTPUT_DIR"])
    
    if not nobin:
        R5REF_args = [python_bin,"%s/Gse/bin/pexpect_runner.py"%build_root,"R5REF.log","R5REF Application",BIN,"-p","%d"%used_port,"-a",addr,"-l"]
        R5REF = subprocess.Popen(R5REF_args)
    
    GUI.wait()

    if not nobin:
        try:
            R5REF.send_signal(signal.SIGTERM)
        except:
            pass
