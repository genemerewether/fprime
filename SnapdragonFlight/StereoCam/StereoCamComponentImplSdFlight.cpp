// ======================================================================
// \title  StereoCamComponentImplSdFlight.cpp
// \author tcanham
// \author mereweth
// \brief  cpp file for StereoCam component implementation class on Snapdragon
//
// \copyright
...
#define SCAM_TRUNC 1 // truncate 10bit -> 8bit
#define SCAM_SCALE 0 // linearly scale 10bit -> 8bit with rounding

#define SCAM_TEST_NAV_AOI 0
...
namespace SnapdragonFlight {
    ...
    preamble(void)
  {
      if (m_initialized) {
          ...
          this->log_WARNING_HI_SCAM_AlreadyInitializedError();
          return;
      }
      ...
      if (numCameras < 1) {
          ...
          this->log_WARNING_HI_SCAM_CameraError(SCAM_NO_CAMERA);
          return;
      }
      ...
      for (NATIVE_INT_TYPE i = 0; i < numCameras; i++) {
          ...
          if (cameraInfo.func == static_cast<NATIVE_INT_TYPE>(SCAM_CAMERA_TYPE))
          {
              cameraID = i;
              found = true;
          }
...
#else
        cameraID = static_cast<NATIVE_INT_TYPE>(SCAM_CAMERA_TYPE);
#endif
      ...
      if (stat) {
          ...
          this->log_WARNING_HI_SCAM_CameraError(SCAM_PARAM_ERR);
          return;
      }

      const camera::ImageSize imageSize = camera::ImageSize(
          static_cast<NATIVE_UINT_TYPE>(SCAM_IMAGE_WIDTH),
          static_cast<NATIVE_UINT_TYPE>(SCAM_IMAGE_HEIGHT));
      ...
      */
      ...
      m_mvCPAConfig.width  = static_cast<NATIVE_UINT_TYPE>(SCAM_AOI_WIDTH);
      m_mvCPAConfig.height = static_cast<NATIVE_UINT_TYPE>(SCAM_AOI_HEIGHT);
      ...
      DEBUG_PRINT("Initialized StereoCam with raw size %s\n", m_params.get("raw-size").c_str());
  ...
  {
      if (m_activated) {
          DEBUG_PRINT("Deactivating StereoCam in finalizer\n");
          deactivate();
      }
    ...
    activate(void)
  {
      ...
      // clear throttles
      this->log_WARNING_HI_SCAM_NoHPBuffers_ThrottleClear();
      this->log_ACTIVITY_HI_SCAM_NoLPBuffers_ThrottleClear();
      this->log_WARNING_HI_SCAM_LoRate_ThrottleClear();
      this->log_WARNING_HI_SCAM_MaxExposureDelta_ThrottleClear();
      ...
      if (stat) {
          ...
          this->log_WARNING_HI_SCAM_CameraError(SCAM_UNKNOWN_ERR);
          return stat;
      }
      ...
      DEBUG_PRINT("\nActivating StereoCamComponentImpl at time %f\n",
    ...
    deactivate(void)
  {
      ...
      struct timeval tv;
      gettimeofday(&tv,NULL);
      DEBUG_PRINT("\nDeactivating StereoCamComponentImpl at time %f\n",
      tv.tv_sec + tv.tv_usec / 1000000.0);
#endif
    ...
    setCallbackType(CallbackType type)
  {
      switch (type) {
          case CALLBACK_8BIT:
              /* NOTE(mereweth@jpl.nasa.gov) - we only save SCAM_IMAGE_SIZE bytes;
  ...
  {
      if (!m_activated) {
          this->log_WARNING_LO_SCAM_ExtraPreviewCallbackError();
          DEBUG_PRINT("\nonPreviewFrame called while StereoCam (de)activating\n");
          return;
      }
      ...
      switch (m_callbackType) {
          case CALLBACK_8BIT:
              callbackSize = static_cast<NATIVE_UINT_TYPE>(SCAM_8BIT_CALLBACK_SIZE);
              break;
          case CALLBACK_10BIT:
              callbackSize = static_cast<NATIVE_UINT_TYPE>(SCAM_10BIT_CALLBACK_SIZE);
              break;
          default:
              DEBUG_PRINT("Unhandled camera callback type %d\n", m_callbackType);
      ...
      if ((frame == NULL)              ||
          ...
          (frame->data == NULL))        {
          this->log_WARNING_HI_SCAM_CameraError(SCAM_IMG_EMPTY);
      ...
      //float correction = 1e3 * ((float)m_exposureTargetTrunc * SCAM_DEFAULT_ROW_PERIOD_US / 2.f);
      ...
      if (m_imagesAcquired % (SCAM_PARAMS_APPLY_SKIP+1) == 0) {
          ...
          mvCPA_AddFrame(m_mvCPAPtr,
                         ...
                         + static_cast<NATIVE_UINT_TYPE>(SCAM_AOI_START_ROW * SCAM_IMAGE_WIDTH
                             * ((CALLBACK_10BIT == m_callbackType) ? 10 / 8 : 1)
                             + SCAM_AOI_START_COL),
                         static_cast<NATIVE_UINT_TYPE>(SCAM_AOI_STRIDE));
          ...
          if ((cpaExposure < m_exposureMin)  ||
              ...
              if (m_exposureMode != EXPOSURE_FIXED) {
                  if (tempExposureTrunc > m_exposureTargetTrunc + m_maxExposureDelta) {
                      if (m_flightMode == SCAM_MODE_FLIGHT) {
                          ++m_numMaxExposureDelta;
                          this->log_WARNING_HI_SCAM_MaxExposureDelta(m_exposureTargetTrunc,
                                                                     tempExposureTrunc);
      ...
      if (m_callbackType == CALLBACK_10BIT) {
          ...
          for (NATIVE_UINT_TYPE dst = 0; dst < SCAM_IMAGE_SIZE; dst+=4) {
#if SCAM_TRUNC
          ...
          if (filp == NULL) {
              ...
              uint8_t *ptr = static_cast<uint8_t *>(frame->data)
                  + static_cast<NATIVE_UINT_TYPE>(SCAM_AOI_START_ROW * SCAM_IMAGE_WIDTH
                      * ((CALLBACK_10BIT == m_callbackType) ? 10 / 8 : 1)
                      + SCAM_AOI_START_COL);
              for (int i = 0; i < SCAM_AOI_HEIGHT; i++) {
                  fwrite((const void*)ptr,
                         1,
                         static_cast<NATIVE_UINT_TYPE>(SCAM_AOI_WIDTH),
                         filp);
                  ptr += static_cast<NATIVE_UINT_TYPE>(SCAM_AOI_STRIDE);
              }
      ...
      if (m_frameCounter >= m_frameSkip) {
          ...
          for(PortOutType i = SCAM_OUT_MIN; i < SCAM_OUT_MAX; i = (PortOutType) ((int) i+1)) {
              ...
              if (((i == SCAM_GNC_OUT) &&
                  ...
                  ((i == SCAM_IMG_TLM_OUT) &&
                   ...
                   (m_imgTlmMode == SCAM_IMG_TLM_ON) &&
                   (m_saveNextFrameProc   ||
                    m_saveNextFrameUnproc ||
                    (0 == m_imagesAcquired %
                     static_cast<NATIVE_UINT_TYPE>(SCAM_IMG_TLM_SKIP))))) {

                  if (buffType == 1) {
                      DEBUG_PRINT("\nSCAM sending LP buffer; m_logMode: %d; m_saveNextFrameUnproc: %d; m_saveNextFrameProc: %d\n",
                  ...
                  if (entryFound) {
                      ...
                      memcpy((void*)ptr, frame->data,
                             static_cast<size_t>(SCAM_IMAGE_SIZE));
                      ...
                      switch (i) {
                          ...
                          case SCAM_PROC_OUT:
                          {
                              ...
                              if (i == SCAM_PROC_OUT) {
                                  DEBUG_PRINT("\nStereoCam Sending image out on image port as CameraFrame; i = %d\n", i);
                                  ...
                                  DEBUG_PRINT("\nStereoCam Sending image out on unproc port as Fw::Buffer; i = %d\n", i);
                                  this->UnprocSend_out(0, buff);
                              }
                          }
                              break;
                          case SCAM_IMG_TLM_OUT:
                              DEBUG_PRINT("\nStereoCam Sending image out on tlm port as Fw::Buffer; i = %d\n", i);
                      ...
                      if (buffType == 0) { // high-priority
                          this->log_WARNING_HI_SCAM_NoHPBuffers(0);
                          this->tlmWrite_SCAM_NumNoHPBuffers(++m_numNoHPBuffers);
                      }
                      else { // low-priority
                          this->log_ACTIVITY_HI_SCAM_NoLPBuffers(i - 1);
                          this->tlmWrite_SCAM_NumNoLPBuffers(++m_numNoLPBuffers);
                      }
                      DEBUG_PRINT("\nSCAM no buffers for port %d\n", i);
                  }
          ...
          if (m_waitingImage) {
              ...
              this->log_ACTIVITY_HI_SCAM_TookImg();
      ...
      if (this->isConnected_FileLogger_OutputPort(0)) {
          ...
          fileBuff.resetSer();
          stat = fileBuff.serialize((U8)AFL_SCAM_CALLBACK);
