#ifndef _UT_HEXRTR_STUB_H
#define _UT_HEXRTR_STUB_H
#include "ut_hexrtr.h"
#ifndef _QAIC_ENV_H
#define _QAIC_ENV_H
...
#ifndef _UT_HEXRTR_SLIM_H
#define _UT_HEXRTR_SLIM_H
#include "remote.h"
#include <stdint.h>
...
#ifndef _const_ut_hexrtr_handle
#define _const_ut_hexrtr_handle ((remote_handle)-1)
#endif //_const_ut_hexrtr_handle

static void _ut_hexrtr_pls_dtor(void* data) {
   remote_handle* ph = (remote_handle*)data;
   if(_const_ut_hexrtr_handle != *ph) {
      (void)__QAIC_REMOTE(remote_handle_close)(*ph);
      *ph = _const_ut_hexrtr_handle;
   }
}

static int _ut_hexrtr_pls_ctor(void* ctx, void* data) {
   remote_handle* ph = (remote_handle*)data;
   *ph = _const_ut_hexrtr_handle;
...
__QAIC_STUB_EXPORT remote_handle _ut_hexrtr_handle(void) {
   remote_handle* ph;
   if(adsp_pls_add_lookup) {
      if(0 == adsp_pls_add_lookup((uint32_t)_ut_hexrtr_handle, 0, sizeof(*ph),  _ut_hexrtr_pls_ctor, "ut_hexrtr",  _ut_hexrtr_pls_dtor, (void**)&ph))  {
         return *ph;
      }
      return (remote_handle)-1;
   } else if(HAP_pls_add_lookup) {
      if(0 == HAP_pls_add_lookup((uint32_t)_ut_hexrtr_handle, 0, sizeof(*ph),  _ut_hexrtr_pls_ctor, "ut_hexrtr",  _ut_hexrtr_pls_dtor, (void**)&ph))  {
         return *ph;
      }
      return (remote_handle)-1;
   }
...
uint32_t _ut_hexrtr_atomic_CompareAndExchange(uint32_t * volatile puDest, uint32_t uExchange, uint32_t uCompare);
...
__QAIC_STUB_EXPORT remote_handle _ut_hexrtr_handle(void) {
   static remote_handle handle = _const_ut_hexrtr_handle;
   if((remote_handle)-1 != handle) {
      return handle;
   } else {
      remote_handle tmp;
      int nErr = _ut_hexrtr_pls_ctor("ut_hexrtr", (void*)&tmp);
      if(nErr) {
         return (remote_handle)-1;
      }
      if(((remote_handle)-1 != handle) || ((remote_handle)-1 != (remote_handle)_ut_hexrtr_atomic_CompareAndExchange((uint32_t*)&handle, (uint32_t)tmp, (uint32_t)-1))) {
         _ut_hexrtr_pls_dtor(&tmp);
      }
      return handle;
   }
}
...
__QAIC_STUB_EXPORT int __QAIC_STUB(ut_hexrtr_skel_invoke)(uint32_t _sc, remote_arg* _pra) __QAIC_STUB_ATTRIBUTE {
   return __QAIC_REMOTE(remote_handle_invoke)(_ut_hexrtr_handle(), _sc, _pra);
}
...
__QAIC_STUB_EXPORT int __QAIC_STUB(ut_hexrtr_run)(void) __QAIC_STUB_ATTRIBUTE {
   uint32_t _mid = 0;
   return _stub_method(_ut_hexrtr_handle(), _mid);
}
...
__QAIC_STUB_EXPORT int __QAIC_STUB(ut_hexrtr_rpc_relay_port_read)(unsigned int* port, unsigned char* buff, int buffLen, int* bytes) __QAIC_STUB_ATTRIBUTE {
   uint32_t _mid = 1;
   return _stub_method_1(_ut_hexrtr_handle(), _mid, (uint32_t*)port, (char**)&buff, (uint32_t*)&buffLen, (uint32_t*)bytes);
}
__QAIC_STUB_EXPORT int __QAIC_STUB(ut_hexrtr_rpc_relay_buff_read)(unsigned int* port, unsigned char* buff, int buffLen, int* bytes) __QAIC_STUB_ATTRIBUTE {
   uint32_t _mid = 2;
   return _stub_method_1(_ut_hexrtr_handle(), _mid, (uint32_t*)port, (char**)&buff, (uint32_t*)&buffLen, (uint32_t*)bytes);
}
...
__QAIC_STUB_EXPORT int __QAIC_STUB(ut_hexrtr_rpc_relay_write)(unsigned int port, const unsigned char* buff, int buffLen) __QAIC_STUB_ATTRIBUTE {
   uint32_t _mid = 3;
   return _stub_method_2(_ut_hexrtr_handle(), _mid, (uint32_t*)&port, (char**)&buff, (uint32_t*)&buffLen);
}
#ifdef __cplusplus
}
#endif
#endif //_UT_HEXRTR_STUB_H
