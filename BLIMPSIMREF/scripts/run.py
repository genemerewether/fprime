...
#     print 'addr = ', addr 
    ...
    # run Gse GUI
    GUI_args = [python_bin,"%s/Gse/bin/gse.py"%build_root,"--port","%d"%used_port,"--dictionary","%s/Gse/generated/BLIMPSIMREF"%build_root,"--connect","--addr",addr,"-L","%s/BLIMPSIMREF/logs"%build_root]
    ...
    BIN = "%s/BLIMPSIMREF/%s/BLIMPSIMREF"%(build_root,os.environ["OUTPUT_DIR"])
    
    if not nobin:
        BLIMPSIMREF_args = [python_bin,"%s/Gse/bin/pexpect_runner.py"%build_root,"BLIMPSIMREF.log","BLIMPSIMREF Application",BIN,"-p","%d"%used_port,"-a",addr,"-l"]
        BLIMPSIMREF = subprocess.Popen(BLIMPSIMREF_args)
    
    GUI.wait()

    if not nobin:
        try:
            BLIMPSIMREF.send_signal(signal.SIGTERM)
        except:
            pass
