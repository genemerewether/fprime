...
#     print 'addr = ', addr 
    ...
    # run Gse GUI
    GUI_args = [python_bin,"%s/Gse/bin/gse.py"%build_root,"--port","%d"%used_port,"--dictionary","%s/Gse/generated/BLIMPHEXREF"%build_root,"--connect","--addr",addr,"-L","%s/BLIMPHEXREF/logs"%build_root]
    ...
    BIN = "%s/BLIMPHEXREF/%s/BLIMPHEXREF"%(build_root,os.environ["OUTPUT_DIR"])
    
    if not nobin:
        BLIMPHEXREF_args = [python_bin,"%s/Gse/bin/pexpect_runner.py"%build_root,"BLIMPHEXREF.log","BLIMPHEXREF Application",BIN,"-p","%d"%used_port,"-a",addr,"-l"]
        BLIMPHEXREF = subprocess.Popen(BLIMPHEXREF_args)
    
    GUI.wait()

    if not nobin:
        try:
            BLIMPHEXREF.send_signal(signal.SIGTERM)
        except:
            pass
