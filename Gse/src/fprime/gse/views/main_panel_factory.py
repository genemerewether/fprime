...
from fprime.gse.views import image_panel
...
class MainPanelFactory:
    ...
    def create(self, root, top=True):
        ...
        # Create image panel
        #
        img_panel = image_panel.ImagePanel(the_panel.p11(), the_panel)
        the_panel.register_panel(img_panel)
        #
        # Get / create an image_listener singlton/observer to
        # update the image area of all instanced panels.
        #
        img_listen = image_listener.ImageListener.getInstance()
        img_listen.addObserver(img_panel)
        if top:
            img_listen.setupLogging(self.__opts)
