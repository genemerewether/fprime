...
class FPrimeTestCase(fprime.gse.utils.test_api.TestApi, unittest.TestCase):
    ...
    def get_config(self, key):
        '''
        Gets the specified configuration kwy or fails the active test because it was not found
        @param key: key to return
        '''
        try:
            return self.get_testbed_config()[key]
    ...
    def get_latest_tlm(self, channel, timeout=5):
        ...
        tlm_list, dumby = self.wait_tlm(channel, timeout=timeout)
    ...
    def get_latest_evr(self, evr, timeout=5, chained=False):
        ...
        dumby, evr_list = self.wait_evr(evr, timeout=timeout)
        return [evr_entry[1] for evr_entry in evr_list if evr_entry[0] == eid]

    def check_tlm(self, tlm, value):
        '''
