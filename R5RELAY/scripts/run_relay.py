...
#     print 'addr = ', addr 
    ...
    # run Gse GUI
    GUI_args = [python_bin,"%s/Gse/bin/gse.py"%build_root,"--port","%d"%used_port,"--dictionary","%s/Gse/generated/R5RELAY"%build_root,"--connect","--addr",addr,"-L","%s/R5RELAY/logs"%build_root]
    ...
    BIN = "%s/R5RELAY/%s/R5RELAY"%(build_root,os.environ["OUTPUT_DIR"])
    
    if not nobin:
        R5RELAY_args = [python_bin,"%s/Gse/bin/pexpect_runner.py"%build_root,"R5RELAY.log","R5RELAY Application",BIN,"-p","%d"%used_port,"-a",addr,"-l"]
        R5RELAY = subprocess.Popen(R5RELAY_args)
    
    GUI.wait()

    if not nobin:
        try:
            R5RELAY.send_signal(signal.SIGTERM)
        except:
            pass
