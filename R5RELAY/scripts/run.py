...
#     print 'addr = ', addr 
    ...
    # run Gse GUI
    GUI_args = [python_bin,"%s/Gse/bin/gse.py"%build_root,"--port","%d"%used_port,"--dictionary","%s/R5RELAY/py_dict"%build_root,"--connect","--addr",addr,"-L","%s/R5RELAY/logs"%build_root]
    ...
    bin_path = "%s/R5RELAY/%s/R5RELAY"%(build_root,os.environ["OUTPUT_DIR"])
    
    # run as root
    if not nobin:
        bin_args = [python_bin, "%s/Gse/bin/pexpect_runner.py"%build_root,"R5RELAY.log","R5RELAY Application","sudo",bin_path,"-p","%d"%used_port,"-a",addr]
