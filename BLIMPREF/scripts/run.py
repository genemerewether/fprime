...
#     print 'addr = ', addr 
    ...
    # run Gse GUI
    GUI_args = [python_bin,"%s/Gse/bin/gse.py"%build_root,"--port","%d"%used_port,"--dictionary","%s/Gse/generated/BLIMPREF"%build_root,"--connect","--addr",addr,"-L","%s/BLIMPREF/logs"%build_root]
    ...
    BIN = "%s/BLIMPREF/%s/BLIMPREF"%(build_root,os.environ["OUTPUT_DIR"])
    
    if not nobin:
        BLIMPREF_args = [python_bin,"%s/Gse/bin/pexpect_runner.py"%build_root,"BLIMPREF.log","BLIMPREF Application",BIN,"-p","%d"%used_port,"-a",addr,"-l"]
        BLIMPREF = subprocess.Popen(BLIMPREF_args)
    
    GUI.wait()

    if not nobin:
        try:
            BLIMPREF.send_signal(signal.SIGTERM)
        except:
            pass
