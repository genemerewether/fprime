...
#     print 'addr = ', addr 
    ...
    # run Gse GUI
    GUI_args = [python_bin,"%s/Gse/bin/gse.py"%build_root,"--port","%d"%used_port,"--dictionary","%s/CARREF/py_dict"%build_root,"--connect","--addr",addr,"-L","%s/CARREF/logs"%build_root]
    ...
    bin_path = "%s/CARREF/%s/CARREF"%(build_root,os.environ["OUTPUT_DIR"])
    
    # run as root
    if not nobin:
        bin_args = [python_bin, "%s/Gse/bin/pexpect_runner.py"%build_root,"CARREF.log","CARREF Application","sudo",bin_path,"-p","%d"%used_port,"-a",addr]
