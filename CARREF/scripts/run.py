...
#     print 'addr = ', addr 
    ...
    # run Gse GUI
    GUI_args = [python_bin,"%s/Gse/bin/gse.py"%build_root,"--port","%d"%used_port,"--dictionary","%s/Gse/generated/CARREF"%build_root,"--connect","--addr",addr,"-L","%s/CARREF/logs"%build_root]
    ...
    BIN = "%s/CARREF/%s/CARREF"%(build_root,os.environ["OUTPUT_DIR"])
    
    if not nobin:
        CARREF_args = [python_bin,"%s/Gse/bin/pexpect_runner.py"%build_root,"CARREF.log","CARREF Application",BIN,"-p","%d"%used_port,"-a",addr,"-l"]
        CARREF = subprocess.Popen(CARREF_args)
    
    GUI.wait()

    if not nobin:
        try:
            CARREF.send_signal(signal.SIGTERM)
        except:
            pass
