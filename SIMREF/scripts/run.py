...
#     print 'addr = ', addr 
    ...
    # run Gse GUI
    GUI_args = [python_bin,"%s/Gse/bin/gse.py"%build_root,"--port","%d"%used_port,"--dictionary","%s/SIMREF/py_dict"%build_root,"--connect","--addr",addr,"-L","%s/SIMREF/logs"%build_root]
    ...
    bin_path = "%s/SIMREF/%s/SIMREF"%(build_root,os.environ["OUTPUT_DIR"])
    
    # run as root
    if not nobin:
        bin_args = [python_bin, "%s/Gse/bin/pexpect_runner.py"%build_root,"SIMREF.log","SIMREF Application","sudo",bin_path,"-p","%d"%used_port,"-a",addr]
