...
#include <Svc/FileDownlink/FileDownlinkCfg.hpp>
...
namespace Svc {
    ...
    )
  {
      ...
      if (this->curTimer >= this->timeout && this->mode.get() == Mode::WAIT) {
          this->curTimer = 0;
          this->log_WARNING_HI_FileDownlink_DownlinkTimeout(this->file.sourceName, this->file.destName);
          this->enterIdle();
      ...
      //If we are running based on a sched call and not a back-to-back transfer attempt new transfer
      if (FILEDOWNLINK_PACKETS_BY_RUN) {
          downlinkPacket();
      }
  }
    ...
    )
  {
	  ...
	  if (this->lastBufferId != fwBuffer.getbufferID() + 1 ||
      ...
      if (not FILEDOWNLINK_PACKETS_BY_RUN) {
          this->downlinkPacket();
      }
  }
    ...
    )
  {
    ...
    this->log_ACTIVITY_HI_FileDownlink_SendStarted(this->file.size, this->file.sourceName, this->file.destName);
  }

  void FileDownlink ::
    FILE_DWN_SEND_PARTIAL_cmdHandler(
      FwOpcodeType opCode,
      U32 cmdSeq,
      const Fw::CmdStringArg& sourceFileName,
      const Fw::CmdStringArg& destFileName,
   ...
   )
  {
    ...
    if (startOffset >= this->file.size) {
        this->enterIdle();
        this->log_WARNING_HI_FileDownlink_DownlinkPartialFail(this->file.sourceName, this->file.destName, startOffset, this->file.size);
        this->cmdResponse_out(opCode, cmdSeq, Fw::COMMAND_VALIDATION_ERROR);
        return;
    } else if (startOffset + length > this->file.size) {
        this->log_WARNING_LO_FileDownlink_DownlinkPartialWarning(startOffset, length, this->file.size, this->file.sourceName, this->file.destName);
    ...
    if (length > 0) {
        this->log_ACTIVITY_HI_FileDownlink_SendStarted(length, this->file.sourceName, this->file.destName);
        this->endOffset = startOffset + length;
    }
    else {
        this->log_ACTIVITY_HI_FileDownlink_SendStarted(this->file.size - startOffset, this->file.sourceName, this->file.destName);
        this->endOffset = this->file.size;
    }
  }

  void FileDownlink ::
    FILE_DWN_CANCEL_cmdHandler(
        const FwOpcodeType opCode,
        const U32 cmdSeq
    )
  {
    ...
    downlinkPacket()
  {
      ...
      //If in downlink mode and currently downlinking data then continue with the next packer
          ...
          if (status != Os::File::OP_OK) {
              this->log_WARNING_HI_FileDownlink_SendDataFail(this->file.sourceName, this->byteOffset);
              this->enterIdle();
              this->cmdResponse_out(this->curOpCode, this->curCmdSeq, Fw::COMMAND_EXECUTION_ERROR);
    ...
    finishHelper(bool cancel)
  {
      ...
      if (not cancel) {
          ...
          this->log_ACTIVITY_HI_FileDownlink_FileSent(this->file.sourceName, this->file.destName);
      } else {
          this->log_ACTIVITY_HI_FileDownlink_DownlinkCanceled(this->file.sourceName, this->file.destName);
      }
      this->enterIdle();
    ...
    getBuffer(Fw::Buffer& buffer, PacketType type)
  {
      ...
      buffer.setdata(reinterpret_cast<POINTER_CAST>(this->memoryStore[type]));
      buffer.setsize(FILEDOWNLINK_INTERNAL_BUFFER_SIZE);
      //Set a known ID to look for later
      buffer.setbufferID(lastBufferId);
      lastBufferId++;
  }
} // end namespace Svc
