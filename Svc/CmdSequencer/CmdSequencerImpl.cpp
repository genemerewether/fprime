...
namespace Svc {
    ...
    void CmdSequencerComponentImpl::schedIn_handler(NATIVE_INT_TYPE portNum,
        ...
        if (this->m_cmdTimer.isExpiredAt(currTime)) {
            this->comCmdOut_out(m_cmdPortNum, m_cmdRecord.m_command, 0);
            this->m_cmdTimer.clear();
    ...
    void CmdSequencerComponentImpl::performCmd_Step(void) {
        ...
        Fw::CmdPacket cmdPkt;
        stat = cmdPkt.deserialize(this->m_cmdRecord.m_command);
        if (stat == Fw::FW_SERIALIZE_OK) {
            this->m_cmdPortNum = this->portFromOpcode(cmdPkt.getOpCode());
        }
        else {
            this->log_WARNING_HI_CS_NoOpcode(this->m_logFileName,
                                             this->m_executedCount,
                                             stat);
        }
        this->m_cmdRecord.m_command.resetDeser();
