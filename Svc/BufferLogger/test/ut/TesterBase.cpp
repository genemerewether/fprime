...
namespace Svc {
...
#endif
  {
    ...
    this->tlmHistory_BL_NumLoggedBuffers = 
      new History<TlmEntry_BL_NumLoggedBuffers>(maxHistorySize);
    this->tlmHistory_BL_NumLoggedBytes = 
      new History<TlmEntry_BL_NumLoggedBytes>(maxHistorySize);
    ...
    this->eventHistory_BL_FileFlushError =
      new History<EventEntry_BL_FileFlushError>(maxHistorySize);
    ...
    ~BufferLoggerTesterBase(void) 
  {
    ...
    delete this->tlmHistory_BL_NumLoggedBuffers;
    delete this->tlmHistory_BL_NumLoggedBytes;
    ...
    delete this->eventHistory_BL_FileFlushError;
  }
    ...
    )
  {
    ...
    switch (id - idBase) {
      ...
      case BufferLoggerComponentBase::CHANNELID_BL_NUMLOGGEDBUFFERS:
      {
        U32 arg;
        const Fw::SerializeStatus _status = val.deserialize(arg);
        if (_status != Fw::FW_SERIALIZE_OK) {
          printf("Error deserializing BL_NumLoggedBuffers: %d\n", _status);
          return;
        }
        this->tlmInput_BL_NumLoggedBuffers(timeTag, arg);
        break;
      }

      case BufferLoggerComponentBase::CHANNELID_BL_NUMLOGGEDBYTES:
      {
        U32 arg;
        const Fw::SerializeStatus _status = val.deserialize(arg);
        if (_status != Fw::FW_SERIALIZE_OK) {
          printf("Error deserializing BL_NumLoggedBytes: %d\n", _status);
          return;
        }
        this->tlmInput_BL_NumLoggedBytes(timeTag, arg);
        break;
      }
  ...
  {
    this->tlmSize = 0;
    this->tlmHistory_BL_NumLoggedBuffers->clear();
    this->tlmHistory_BL_NumLoggedBytes->clear();
  }
  ...
  void BufferLoggerTesterBase ::
    tlmInput_BL_NumLoggedBuffers(
        const Fw::Time& timeTag,
        const U32& val
    )
  {
    TlmEntry_BL_NumLoggedBuffers e = { timeTag, val };
    this->tlmHistory_BL_NumLoggedBuffers->push_back(e);
    ++this->tlmSize;
  }
  ...
  void BufferLoggerTesterBase ::
    tlmInput_BL_NumLoggedBytes(
        const Fw::Time& timeTag,
        const U32& val
    )
  {
    TlmEntry_BL_NumLoggedBytes e = { timeTag, val };
    this->tlmHistory_BL_NumLoggedBytes->push_back(e);
    ++this->tlmSize;
  }
    ...
    )
  {
    ...
    switch (id - idBase) {
      ...
      case BufferLoggerComponentBase::EVENTID_BL_FILEFLUSHERROR: 
      {
        ...
        this->logIn_WARNING_HI_BL_FileFlushError(file, status);
    ...
    clearEvents(void)
  {
    ...
    this->eventHistory_BL_FileFlushError->clear();
  }
  ...
  void BufferLoggerTesterBase ::
    logIn_WARNING_HI_BL_FileFlushError(
        Fw::LogStringArg& file,
        U32 status
    )
  {
    EventEntry_BL_FileFlushError e = {
      file, status
    };
    eventHistory_BL_FileFlushError->push_back(e);
    ++this->eventsSize;
  }
